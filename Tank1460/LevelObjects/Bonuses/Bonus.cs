...
public class Bonus : LevelObject
{
    ...
    private void ApplyEffectOnEnemy(EnemyTank enemyTank)
    {
        Level.SoundPlayer.Play(Sound.BonusPickup);
        ...
        {
            case BonusType.Armor:
                enemyTank.AddTimedInvulnerability(600 * Tank1460Game.OneFrameSpan);
                break;

            case BonusType.OneUp:
                Level.EnemyManager.AddOneUp();
                break;
        }
    }
    ...
    private void HandleCollisions()
    {
        ...
        if (!Level.EnemiesCanGrabBonuses)
            return;

        var enemy = allTankCollisions.OfType<EnemyTank>().FirstOrDefault();
