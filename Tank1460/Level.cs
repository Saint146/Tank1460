...
public class Level : IDisposable
{
    ...
    public int EnemySpawnsRemaining => EnemyManager?.SpawnsRemaining ?? 0;

    public bool EnemiesCanGrabBonuses = true;
    ...
    private const int TotalEnemies = 20;
    private const int MaxEnemiesOnScreen = 4;
    ...
    public Level(IServiceProvider serviceProvider, LevelStructure levelStructure, int levelNumber)
    {
        ...
        EnemyManager = new EnemyManager(this, TotalEnemies, MaxEnemiesOnScreen);
    ...
    public PlayerTank GetTargetPlayerForEnemy(int enemyIndex)
    {
        return _players.Count switch
        {
            0 => null,
            1 => enemyIndex % 2 == 0 ? null : _players[0],
            _ => _players[enemyIndex % _players.Count]
        };
    }
    ...
    private Tile CreateTile(TileType tileType, int x, int y)
    {
        return tileType switch
        {
            ...
            TileType.Start1 => CreatePlayerSpawn(x, y, 1),

            // Player 2 start point
            TileType.Start2 => CreatePlayerSpawn(x, y, 2),

            // Enemy spawn point
            TileType.StartEnemy => CreateEnemySpawn(x, y),
    ...
    private Tile CreateEnemySpawn(int x, int y)
    {
        EnemyManager.AddSpawnPoint(x, y);
        return null;
    }
    ...
    public void Update(GameTime gameTime, KeyboardState keyboardState)
    {
        ...
        if (!IsGamePaused)
        {
            ...
            EnemyManager.Update(gameTime, keyboardState);
    ...
    public void HandleAllEnemiesDestroyed()
    {
    }
}